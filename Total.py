...
if not st.session_state.logged_in:
    ...
    # Add custom CSS for background color and responsive design
    st.markdown("""
        <style>
            body {
                font-family: Arial, sans-serif;
                background-color: #d3d3d3;  /* Set the background to grey */
                margin: 0;
                padding: 20px;
            }
            .container {
        ...
        if st.button("Match Resumes"):
            if not resume_files or not job_description:
                ...
                for resume, score, pred_score, suggestion in zip(top_resumes, similarity_scores, prediction_scores, suggestions):
                    st.write(f"{resume}:  Prediction Score: {pred_score}")
